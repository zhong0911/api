...
// hansCount("saorao");
$phone = isset($_GET['phone']) && $_GET['phone'] != '' ? $_GET['phone'] : exit(json_encode(['success' => false, 'message' => '参数不完整'], JSON_UNESCAPED_UNICODE));
strlen($phone) != 11 && exit(json_encode(['success' => false, 'message' => '手机号码不正确'], JSON_UNESCAPED_UNICODE));
// 百度的
...
$res_arr = ['success' => true, 'tel' => $phone, 'info' => ['province' => $_resJSON[$phone]['location']['province'], 'city' => $_resJSON[$phone]['location']['area'][0]['city'], 'operator' => $_resJSON[$phone]['location']['operator']], 'data' => [['name' => '360手机卫士', 'msg' => $wawaJson], ['name' => '搜狗号码通', 'msg' => $_sougouOr], ['name' => '百度手机卫士', 'msg' => $_resJSON[$phone]['reports'][0]['id'] ? $_resJSON[$phone]['reports'][0]['name'] : '正常号码']]];
